 ...
 */
@WebServlet(name = "NovaConsulta", urlPatterns = {"/NovaConsultaServlet"})
public class NovaConsultaServlet extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException, IllegalAccessException, InvocationTargetException {
        request.setCharacterEncoding("UTF-8");
        NovoConsultaFormBean npfb = new NovoConsultaFormBean();
        BeanUtils.populate(npfb, request.getParameterMap());
  request.getSession().setAttribute("novoConsulta", npfb);
